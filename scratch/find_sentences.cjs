...
// Filter strings that look like actual phrases or words (not random code/garbage)
...
const filtered = strings.filter(s => {
  ...
  // If it has too many symbols or numbers, ignore
  if ((lower.match(/[^a-zA-Z\s0-9]/g) || []).length > 2) return false;
  return true;
});

console.log("--- POTENTIAL SENTENCES / TEXT RUN VALUES ---");
filtered.forEach(s => {
