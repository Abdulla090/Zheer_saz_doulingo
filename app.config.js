module.exports = ({ config }) => {
  ...
  finalConfig.plugins = (finalConfig.plugins ?? []).map((plugin) => {
    ...
    if (pluginName === "expo-build-properties" && isPreviewBuild) {
      ...
      return [
        pluginName,
        {
          ...
          android: {
            ...(options.android ?? {}),
            // Internal APKs target physical test devices. Production remains
            // an AAB so Google Play can generate the right split per device.
