...
function springMotion(to: number, config = { damping: 15, stiffness: 200 }) {
  if (Platform.OS === "web") {
    return withTiming(to, { duration: 200, easing: Easing.out(Easing.cubic) });
  }
  return withSpring(to, config);
}

export function TwinoHomeScreen({
  onOpenLearningPath,
}: {
  ...
  const talkBtnScale = useSharedValue(1);
  const sheenTranslate = useSharedValue(-200);

  useEffect(() => {
    sheenTranslate.value = withRepeat(
      withSequence(
        withTiming(450, { duration: 1500, easing: Easing.linear }),
  ...
  const animatedTalkBtnStyle = useAnimatedStyle(() => ({
    transform: [{ scale: talkBtnScale.value }],
  }));
  ...
  const handlePressIn = () => {
    talkBtnScale.value = springMotion(0.95);
  };

  const handlePressOut = () => {
    talkBtnScale.value = springMotion(1);
  };
      ...
      >
        ...
        <Animated.View entering={enterFadeDown(40)} style={styles.heroCopy}>
          ...
          <AppText style={styles.headline}>
            I'm <AppText style={styles.headlineAccent}>Twino.</AppText>
          </AppText>
        ...
        <Animated.View entering={enterFadeDown(80)} style={styles.ctaWrap}>
          <Pressable
            ...
            onPress={handleTalk}
            style={({ pressed }) => pressed && Platform.OS === 'web' ? { opacity: 0.9 } : undefined}
          >
            <Animated.View
              style={[
                animatedTalkBtnStyle,
                styles.talkBtn,
                { overflow: "hidden" },
                crossShadow({
                  color: C.blue,
            ...
            >
              ...
              {/* Specular Sheen sweep layer */}
              <Animated.View style={[{ position: "absolute", top: 0, bottom: 0, left: 0, width: 140 }, animatedSheenStyle, { pointerEvents: "none" }]}>
                <LinearGradient
