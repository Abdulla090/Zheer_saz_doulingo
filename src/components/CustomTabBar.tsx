...
function GamesIcon({ size = 26, color = "#B4B8C3" }: { size?: number; color?: string }) {
  return (
    <Svg width={size} height={size} viewBox="0 0 24 24" fill="none">
      <Rect x="4" y="8" width="7" height="10" rx="2" stroke={color} strokeWidth={2} />
      <Rect x="13" y="5" width="7" height="13" rx="2" stroke={color} strokeWidth={2} />
      <Path d="M7.5 11h0M16.5 9h0" stroke={color} strokeWidth={2.5} strokeLinecap="round" />
    </Svg>
  );
}
...
export function CustomTabBar({ state, navigation }: BottomTabBarProps) {
  ...
  const tabBarContent = (
    <>
      ...
      >
        <GamesIcon
          color={activeRouteName === "feed" && !isMoreSheetOpen ? "#208AEF" : "#B4B8C3"}
          size={26}
        />
      </PressableScale>
...
const styles = StyleSheet.create({
  ...
  container: {
    ...
    backgroundColor: Platform.OS === 'web' ? "rgba(255, 255, 255, 0.25)" : "rgba(255, 255, 255, 0.15)",
    borderWidth: 1.5,
